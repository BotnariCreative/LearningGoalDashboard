...
export async function POST(request: Request) {
  ...
  if (!checkRateLimit(`teacher-login:${ip}`)) {
    console.warn(`[auth] Rate limit exceeded for teacher login from ${ip}`)
  ...
  try {
    ...
    if (!valid) {
      console.warn(`[auth] Failed teacher login attempt from ${ip}`)
